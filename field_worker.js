/* field_worker.js : 作業者分析ビュー（請求/直収分離・縦導線安定＋詳細トグル版）
   2026-05-01
   ・件数：作業者CSVの原票番号ユニーク
   ・稼働日：A列日付で判定
   ・売上/直収：M列「請求／直収」で分離
...
'use strict';
(function(){
  if (window.__FIELD_WORKER_VERTICAL_STACK_TOGGLE_FINAL_20260501__) return;
  window.__FIELD_WORKER_VERTICAL_STACK_TOGGLE_FINAL_20260501__ = true;
  ...
  function renderBarRows(items, unitLabel, max){
  ...
  function renderBarList(containerId, items, unitLabel, limit=8){
    ...
    const main = positive.slice(0, limit);
    const rest = positive.slice(limit);
    let html = renderBarRows(main, unitLabel, max);
    if (rest.length) {
      html += `<details class="work-more-toggle">
        <summary>その他 ${rest.length}項目を表示</summary>
        <div class="work-more-body">${renderBarRows(rest, unitLabel, max)}</div>
      </details>`;
    }
    box.innerHTML = html;
  }
  ...
  function renderWorkerDetail(row){
    ...
    renderBarList('worker-size-bars', split.size, '件', 7);
    renderBarList('worker-other-bars', split.other, '件', 8);
    renderBarList('worker-direct-bars', directItems, '件', 8);
  }
