...
(function(){
  ...
  // 現場CSVは field_core.js の FIELD_DATA_ACCESS / STATE に集約し、
  // 別センター混入・削除済みデータ復活・表示遅延を防ぐ。
  ...
  function collectRecords(){
    ...
    // localStorageの保険探索はしない。
    // ここで全探索すると重くなり、別センターや削除済みデータを拾うため。
    return out;
  }
  ...
  function rowsFromProductRecord(rec){
    ...
    tickets.forEach(t=>{
      ...
      const ticketAmount = amountFrom(t);
      const detailAmountTotal = details.reduce((sum,d)=>sum + amountFrom(d), 0);
      ...
      if (details.length) {
        details.forEach((d, idx)=>rows.push({
          ...
          product,
          work: workFrom(d),
          // 旧保存データ対策：明細側の金額が0で、原票合計だけ残っている場合は先頭明細へ寄せる。
          // 新規取込ではfield_core側で明細金額を保持するため、通常はamountFrom(d)が使われる。
          amount: detailAmountTotal > 0 ? amountFrom(d) : (idx === 0 ? ticketAmount : 0)
        }));
      } else {
        rows.push({
          slip,
