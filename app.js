...
/* ════════ §23 REPORT_UI（ChatGPT生成＋編集プレビュー＋Word出力） ═════════════════════════════ */
const REPORT_UI = {
  _OAI_KEY: 'report_oai_key_v1',
  _lastReport: null,

  getKey() {
    try { return localStorage.getItem(this._OAI_KEY) || ''; } catch(e) { return ''; }
  },
  setKey(k) {
    try { localStorage.setItem(this._OAI_KEY, k || ''); } catch(e) {}
  },
  getFY() {
  ...
  getType() {
    return document.getElementById('report-type')?.value || 'monthly';
  },
  ...
  refresh() {
    ...
    if (genBtn && !genBtn._bound) {
      genBtn._bound = true;
      genBtn.addEventListener('click', () => this.generateDraft());
    }

    const wordBtn = document.getElementById('report-word-btn');
    if (wordBtn && !wordBtn._bound) {
      wordBtn._bound = true;
      wordBtn.addEventListener('click', () => this.downloadWordFromPreview());
    }
    ...
    if (copyBtn && !copyBtn._bound) {
      ...
      copyBtn.addEventListener('click', async () => {
        const text = document.getElementById('report-preview-editor')?.value || '';
        if (!text.trim()) { UI.toast?.('コピーする本文がありません'); return; }
        try {
          await navigator.clipboard.writeText(text);
          UI.toast?.('本文をコピーしました');
        } catch(e) {
          const ed = document.getElementById('report-preview-editor');
          ed?.select();
  ...
  resetProgress() {
    this.prog('rpg-1','wait','データを収集');
    this.prog('rpg-2','wait','ChatGPTが文章を生成（20〜40秒）');
    this.prog('rpg-3','wait','プレビューを作成');
    this.prog('rpg-4','wait','Wordライブラリを読み込み');
    this.prog('rpg-5','wait','Wordファイルを作成');
  },

  buildDataPackage(ym) {
    const ds = ym ? activeDatasetByYM(ym) : latestDS();
    const prev = ds ? prevDS(ds.ym) : null;
    const fieldRows = (STATE.fieldData || []).filter(r => !ym || r.ym === ym);
    const productRows = (STATE.productAddressData || []).filter(r => !ym || r.ym === ym);
    const libraryRefs = (STATE.library || []).slice(0, 5).map(x => ({
      title: x.title || x.fileName || '',
      category: x.category || '',
      memo: x.memo || x.summary || ''
    }));
    ...
    const pkg = {
      ...
      ymLabel: ymLabel(ym),
      monthly: ds ? {
        income: ds.totalIncome,
        expense: ds.totalExpense,
        profit: ds.profit,
        profitRate: ds.profitRate,
        pseudoLaborRate: ds.pseudoLaborRate,
        prevIncomeRate: prev ? ratio(ds.totalIncome, prev.totalIncome) : '',
        prevProfitDiff: prev ? (ds.profit - prev.profit) : null,
      } : null,
      dataCounts: {
      ...
      references: libraryRefs,
    };
    return pkg;
  },

  buildDataSummary(ym) {
    const pkg = this.buildDataPackage(ym);
    const m = pkg.monthly;
    const lines = [];
    lines.push(`センター: ${pkg.center}`);
    lines.push(`対象月: ${pkg.ymLabel}`);
    if (m) {
      lines.push(`営業収益: ${fmtK(m.income)}千円`);
      lines.push(`費用合計: ${fmtK(m.expense)}千円`);
      lines.push(`センター利益: ${fmtK(m.profit)}千円`);
      lines.push(`利益率: ${pct(m.profitRate)}`);
      lines.push(`みなし人件費率: ${pct(m.pseudoLaborRate)}`);
      if (m.prevIncomeRate) lines.push(`前月比 営業収益: ${m.prevIncomeRate}`);
    } else {
      lines.push('月次収支データなし');
    }
    lines.push(`現場明細件数: ${pkg.dataCounts.fieldRows}件`);
    lines.push(`商品住所件数: ${pkg.dataCounts.productRows}件`);
    return lines.join('\n');
  },
  ...
  buildPrompt(type, fy, ym, extra) {
    ...
    const pkg = this.buildDataPackage(ym);
    const refs = (pkg.references || []).map((item,i)=>`【資料${i+1}】${item.title}\n区分: ${item.category}\nメモ: ${item.memo || 'なし'}`).join('\n\n');

    return `${CENTER.name}の${typeLabel}を作成してください。

【目的】
役員・部長向けに、そのままWord報告書へ転記できる本文を作ること。

【絶対ルール】
箇条書き記号（・、―、-）は使わない。
表ではなく、流れる文章の段落で書く。
「一方で」「その中で」「これにより」「また」「今後は」などの接続詞でつなぐ。
数字は必ず文中に具体的に入れる。
断定しすぎず、実務報告として自然な表現にする。
確認できない内容は推測で書かない。
JSONのみ返す。
...
【出力JSON形式】
{
  ...
  "summary": "全体概要の段落",
  "review": {
    "heading": "【${prevPeri} 振り返り】または【月次実績】",
    "p1": "実績サマリーの段落",
    "p2": "課題・背景・対応経緯の段落",
    "p3": "取り組み成果・構造変化の段落",
    "p4": "評価と次月または${period}への接続の段落"
  },
  "policy": {
    "heading": "【${period} 運営方針】または【今後の対応】",
    "intro": "方針宣言の段落",
    "items": [
      {"title":"■ 施策タイトル1", "p1":"内容・数値根拠の段落", "p2":"実施方法・見通しの段落"},
      {"title":"■ 施策タイトル2", "p1":"説明段落", "p2":"説明段落"},
      {"title":"■ まとめ", "p1":"全体まとめの段落", "p2":""}
    ]
  }
}
...
${extra || 'なし'}`;
  ...
  _reportToText(type, fy, ym, rpt) {
    const period = reportHalfFromYM(ym);
    const title = rpt.title || `${CENTER.name} ${ymLabel(ym)} 会議報告書`;
    const lines = [];
    lines.push(title);
    lines.push(`${CENTER.name}　${fy}年度　${ymLabel(ym)}`);
    lines.push('');
    if (rpt.summary) { lines.push('【総括】'); lines.push(rpt.summary); lines.push(''); }

    const review = rpt.review || {};
    lines.push(review.heading || (type === 'monthly' ? '【月次実績】' : `【${period === '上期' ? '下半期' : '上半期'} 振り返り】`));
    ['p1','p2','p3','p4'].forEach(k => { if (review[k]) { lines.push(review[k]); lines.push(''); } });

    const policy = rpt.policy || {};
    lines.push(policy.heading || (type === 'monthly' ? '【今後の対応】' : `【${period} 運営方針】`));
    if (policy.intro) { lines.push(policy.intro); lines.push(''); }
    (policy.items || []).forEach(item => {
      if (item.title) lines.push(item.title);
      if (item.p1) lines.push(item.p1);
      if (item.p2) lines.push(item.p2);
      lines.push('');
    });
    return lines.join('\n').replace(/\n{3,}/g, '\n\n').trim();
  },
  ...
  async generateDraft() {
    ...
    if (!key) {
      alert('ChatGPT APIキーを入力・保存してください。');
      return;
    }
    ...
    const editor = document.getElementById('report-preview-editor');
    const msg = document.getElementById('report-output-msg');
    ...
    try {
      if (btn) {
        btn.disabled = true;
        btn.innerHTML = '<span style="display:inline-block;width:14px;height:14px;border:2px solid rgba(255,255,255,.3);border-top-color:#fff;border-radius:50%;animation:spin .7s linear infinite"></span>　AI生成中...';
      }
      ...
      const res = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: 'system', content: 'あなたは物流センターの経営管理報告書を作成する実務担当です。必ずJSONのみで返してください。' },
      ...
      const rpt = this._parseAIJson(text);
      this._lastReport = { type, fy, ym, rpt, createdAt: new Date().toISOString() };
      this.prog('rpg-2','done','文章生成完了');

      this.prog('rpg-3','active','プレビューを作成中...');
      const preview = this._reportToText(type, fy, ym, rpt);
      if (editor) editor.value = preview;
      if (card) card.style.display = '';
      this.prog('rpg-3','done','プレビュー作成完了。内容を確認・修正できます');
      this.prog('rpg-4','wait','Word出力時に読み込み');
      this.prog('rpg-5','wait','Word出力待ち');
      UI.toast?.('AI下書きを生成しました');
    } catch(e) {
      ...
      alert('エラーが発生しました:\n\n' + e.message);
      console.error('[REPORT_AI]', e);
    } finally {
      if (btn) {
        btn.disabled = false;
        btn.innerHTML = '✨　AI下書きを生成する';
      }
    }
  },
  ...
  async ensureDocx() {
    ...
    await new Promise((resolve, reject) => {
      const existing = document.getElementById('docx-iife-script');
      if (existing) {
        existing.addEventListener('load', resolve, { once:true });
        existing.addEventListener('error', () => reject(new Error('docx.iife.js の読み込みに失敗しました。')), { once:true });
        return;
      }
      const s = document.createElement('script');
      s.id = 'docx-iife-script';
      s.src = 'docx.iife.js';
      s.onload = resolve;
      s.onerror = () => reject(new Error('docx.iife.js の読み込みに失敗しました。'));
      document.head.appendChild(s);
    });
  },

  async downloadWordFromPreview() {
    const editor = document.getElementById('report-preview-editor');
    const text = editor?.value?.trim() || '';
    if (!text) { alert('先にAI下書きを生成するか、本文を入力してください。'); return; }
    const msg = document.getElementById('report-output-msg');
    const btn = document.getElementById('report-word-btn');
    try {
      if (btn) { btn.disabled = true; btn.textContent = 'Word作成中...'; }
      if (msg) msg.textContent = 'Wordライブラリを読み込み中...';
      this.prog('rpg-4','active','Wordライブラリを読み込み中...');
      await this.ensureDocx();
      this.prog('rpg-4','done','Wordライブラリ読み込み完了');

      if (msg) msg.textContent = 'Wordファイルを作成中...';
      this.prog('rpg-5','active','Wordファイルを作成中...');
      const blob = await this._buildDocxFromText(text);
      const tag = new Date().toISOString().slice(0,10).replace(/-/g,'');
      const ym = this.getYM();
      const a = document.createElement('a');
      const url = URL.createObjectURL(blob);
      a.href = url;
      a.download = `${CENTER.name}_会議報告書_${ym || tag}.docx`;
      a.click();
      setTimeout(()=>URL.revokeObjectURL(url), 3000);
      this.prog('rpg-5','done','Wordファイル作成完了');
      if (msg) msg.textContent = 'Wordファイルを出力しました。';
      UI.toast?.('Wordファイルを出力しました');
    } catch(e) {
      this.prog('rpg-5','error','エラー: ' + e.message);
      if (msg) msg.textContent = 'エラー: ' + e.message;
      alert('Word出力でエラーが発生しました:\n\n' + e.message);
      console.error('[REPORT_WORD]', e);
    } finally {
      if (btn) { btn.disabled = false; btn.textContent = 'Word出力'; }
    }
  },

  async _buildDocxFromText(text) {
    ...
    const title = lines.shift() || `${CENTER.name} 会議報告書`;
    const subtitle = lines[0] && !lines[0].startsWith('【') ? lines.shift() : '';

    const children = [
      new Paragraph({ alignment: AlignmentType.RIGHT, spacing:{after:80}, children:[new TextRun({ text:new Date().toLocaleDateString('ja-JP',{year:'numeric',month:'long',day:'numeric'}), size:20, color:LGRAY, font:'游明朝' })] }),
      new Paragraph({ alignment: AlignmentType.CENTER, spacing:{after:70}, children:[new TextRun({ text:title, bold:true, size:34, color:NAVY, font:'游明朝' })] }),
    ];
    if (subtitle) children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing:{after:120}, children:[new TextRun({ text:subtitle, size:22, color:LGRAY, font:'游明朝' })] }));
    children.push(new Paragraph({ border:{bottom:{style:BorderStyle.SINGLE,size:6,color:BLUE,space:1}}, spacing:{before:80,after:160}, children:[new TextRun({text:'',size:4})] }));

    lines.forEach(raw => {
      const t = raw.trim();
      if (!t) {
        children.push(new Paragraph({ spacing:{before:0,after:0}, children:[new TextRun({ text:'', size:8 })] }));
      } else if (/^【.+】$/.test(t)) {
        children.push(new Paragraph({ heading: HeadingLevel.HEADING_1, spacing:{before:340,after:100}, children:[new TextRun({ text:t, bold:true, size:28, color:NAVY, font:'游明朝' })] }));
      } else if (/^■/.test(t)) {
        children.push(new Paragraph({ heading: HeadingLevel.HEADING_2, spacing:{before:220,after:70}, children:[new TextRun({ text:t, bold:true, size:23, color:NAVY, font:'游明朝' })] }));
      } else {
        children.push(new Paragraph({ spacing:{before:90,after:90}, indent:{firstLine:420}, children:[new TextRun({ text:t, size:22, color:GRAY, font:'游明朝' })] }));
      }
    });

    const doc = new Document({
      styles: { default: { document: { run: { font:'游明朝', size:22, color:GRAY } } } },
      sections: [{
