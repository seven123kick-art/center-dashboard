...
const STORE = {
  ...
  load() {
    // 大容量CSV本体は localStorage からは読まない。
    // 正本はSupabase、PC内高速キャッシュはIndexedDB。localStorageは軽量設定だけに限定する。
    STATE.datasets  = [];
    STATE.workerCsvData = [];
    ...
    // 旧版で作られた巨大localStorageキーを掃除する。
    ['datasets','workerCsvData','productAddressData','fieldData','areaData'].forEach(k => this._rm(k));
  },

  save() {
    sanitizePersonalDataState(STATE);
    // 大容量CSV本体はlocalStorageへ保存しない。QuotaExceededとセンター切替時の空復元を防ぐ。
    this._s('datasetIndex', (STATE.datasets || []).map(d => ({ ym:d.ym, type:d.type || 'confirmed', source:d.source || 'csv', importedAt:d.importedAt || null, totalIncome:d.totalIncome || 0, totalExpense:d.totalExpense || 0, profit:d.profit || 0 })));
    this._s('workerCsvIndex', (STATE.workerCsvData || []).map(d => this._fieldMeta('worker', d)).filter(Boolean));
    this._s('productAddressIndex', (STATE.productAddressData || []).map(d => this._fieldMeta('product', d)).filter(Boolean));
    ['datasets','workerCsvData','productAddressData','fieldData','areaData'].forEach(k => this._rm(k));
...
/* ════════ §4-B IndexedDB 高速キャッシュ（Supabase正本） ════════════ */
const IDB_CACHE = {
  _db: null,
  _timer: null,
  _dbName: 'mgmt5_center_cache_v1',
  _storeName: 'items',
  async _open() {
  ...
  datasetId(ds) { return ds && ds.ym ? `${ds.ym}_${ds.type || 'confirmed'}` : null; },
  async hydrateState() {
    try {
      const datasetIndex = STORE._g('datasetIndex') || [];
      for (const meta of datasetIndex) {
        const id = `${meta.ym}_${meta.type || 'confirmed'}`;
        const ds = await this.get('dataset', id);
        if (ds && ds.ym) upsertDataset(ds);
      }
      const workerIndex = STORE._g('workerCsvIndex') || [];
      STATE.workerCsvData = [];
      for (const meta of workerIndex) {
        const rec = await this.get('worker', meta.ym);
        if (rec && rec.ym) STATE.workerCsvData.push(rec);
      }
      const productIndex = STORE._g('productAddressIndex') || [];
      STATE.productAddressData = [];
      for (const meta of productIndex) {
        const rec = await this.get('product', meta.ym);
        if (rec && rec.ym) STATE.productAddressData.push(rec);
      }
      if (window.FIELD_DATA_ACCESS?.invalidate) FIELD_DATA_ACCESS.invalidate();
      return { ok:true };
    } catch(e) { console.warn('[IDB_CACHE] hydrate failed', e); return { ok:false, error:e.message }; }
  },
  persistStateSoon() {
    clearTimeout(this._timer);
    this._timer = setTimeout(() => this.persistState(), 250);
  },
  async persistState() {
    try {
      for (const ds of (STATE.datasets || [])) {
        const id = this.datasetId(ds);
        if (id) await this.set('dataset', id, ds);
      }
      for (const w of (STATE.workerCsvData || []).filter(x => x && x.ym)) await this.set('worker', w.ym, w);
      for (const p of (STATE.productAddressData || []).filter(x => x && x.ym)) await this.set('product', p.ym, p);
      return { ok:true };
    } catch(e) { console.warn('[IDB_CACHE] persist failed', e); return { ok:false, error:e.message }; }
  }
};
window.IDB_CACHE = IDB_CACHE;
...
const CLOUD = {
  ...
  _applyFullState(full) {
    ...
    // クラウド上の古い削除フラグは再取込済みデータを消す原因になるため、起動復元時はマージしない。
    sanitizePersonalDataState(STATE);
    applyDeletionTombstonesToState(STATE);
  ...
  async _downloadJSON(key) {
    ...
    if (cacheKind && window.IDB_CACHE) {
      const cached = await IDB_CACHE.get(cacheKind.kind, cacheKind.id);
      // 起動直後の体感速度を優先しつつ、クラウド取得に失敗した場合の保険として使う。
      // ここでは即returnせず、クラウドを正本として取りに行く。
      var cacheFallback = cached || null;
    }
    const sb = await this._client();
    if (!sb) return cacheFallback || null;
    const { data, error } = await sb.storage.from(this._bucket()).download(key);
    if (error) return cacheFallback || null;
  ...
  async pullManifestAndMissing() {
    ...
    // manifest.deleted は古い削除フラグ汚染を招くため、起動・復元時にはマージしない。
    let changed = 0;
  ...
  async pullFieldDataForFiscalYear(fy) {
    ...
    try {
      ...
      const manifest = await this._downloadJSON(this._manifestKey());
      if (!manifest) return { ok:false, error:'manifestなし' };
      // manifest.deleted は古い削除フラグ汚染を招くためマージしない。
  ...
  async pullInitialForBoot(preferredView='dashboard') {
    ...
    try {
      ...
      // manifest.deleted は古い削除フラグ汚染を招くため、起動時にはマージしない。

      const metas = Array.isArray(manifest.datasets) ? manifest.datasets.filter(m => m && m.ym) : [];

      // 起動時に必要な年度は「保存済みデータ全体」から列挙する。
      // latestYmだけで決めると、202604(FY2026)が最新のとき202506〜202603(FY2025)を読み落とす。
      const fySet = new Set();
      if (STATE.fiscalYear) fySet.add(String(STATE.fiscalYear));
      for (const m of metas) {
        if (m && m.ym) fySet.add(String(fiscalYearFromYM(m.ym)));
      }
      ...
      // 並列取得はSupabase/Storageのチャンク取得が詰まりやすいので直列取得にする。
...
function initFiscalYearSelects() {
  ...
  if (!STATE.fiscalYear) STATE.fiscalYear = document.getElementById('plan-year-sel')?.value || defaultFY;
  else {
    const planSelKeep = document.getElementById('plan-year-sel');
    if (planSelKeep && [...planSelKeep.options].some(o => o.value === String(STATE.fiscalYear))) planSelKeep.value = String(STATE.fiscalYear);
  }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    // 1. ローカルストレージから読込
    STORE.load();
    // IndexedDBキャッシュを先に戻す。画面はまだローディング中なので「データなし」のちらつきを出さない。
