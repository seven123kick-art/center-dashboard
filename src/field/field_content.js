...
(function(){
  ...
  function ensureStyles(){
    ...
    st.textContent = `
      ...
      .fc-kpi{background:#fff;border:1px solid var(--border);border-radius:16px;box-shadow:var(--shadow);padding:16px;min-height:104px}.fc-kpi-label{font-size:12px;font-weight:900;color:#334155;margin-bottom:8px}.fc-kpi-target-month .fc-kpi-value{font-size:27px;white-space:nowrap;letter-spacing:-.04em}.fc-kpi-value{font-size:28px;font-weight:900;color:#0f172a;line-height:1.1;white-space:nowrap}.fc-kpi-sub{font-size:12px;color:#8291a7;font-weight:800;margin-top:8px;line-height:1.45}
      ...
      .fc-bar-row{display:grid;grid-template-columns:30px minmax(110px,190px) minmax(160px,1fr) minmax(115px,150px);gap:10px;align-items:center;padding:9px 0;border-bottom:1px solid #edf2f7}.fc-rank{width:24px;height:24px;border-radius:999px;background:#e8f1fb;color:var(--navy);font-weight:900;display:flex;align-items:center;justify-content:center;font-size:12px}.fc-name{font-size:13px;font-weight:900;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.fc-track{height:13px;background:#e5e7eb;border-radius:999px;overflow:hidden}.fc-fill{height:100%;background:var(--navy);border-radius:999px}.fc-val{text-align:right;font-size:12px;font-weight:900}.fc-val span{display:block;color:#8291a7;font-size:11px;margin-top:2px}
  ...
  function render(){
    ...
    pane.innerHTML = `<div class="fc-note">作業内容は、大分類 → 中分類 → 小分類で整理しています。グラフはChart.jsを使わず、HTMLバーで表示しています。</div>
      <div class="fc-kpi-grid">
        ...
        <div class="fc-kpi"><div class="fc-kpi-label">作業内容売上</div><div class="fc-kpi-value">${fmtK(totalAmount)}千円</div><div class="fc-kpi-sub">商品・住所CSV 作業明細ベース</div></div>
        ...
        <div class="fc-kpi"><div class="fc-kpi-label">平均単価</div><div class="fc-kpi-value">${fmt(avg)}円</div><div class="fc-kpi-sub">売上 ÷ 作業明細</div></div>
