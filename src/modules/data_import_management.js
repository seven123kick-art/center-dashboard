...
(function(){
  ...
  async function refresh(){
    ...
    const budgetRow=`<tr><td><b>予算計画</b><div class="dim-history">SKFL0001 / PLAN_BUDGET</div></td><td>${plan?'<span class="dim-state is-current">登録済</span>':'<span class="dim-state is-missing">未登録</span>'}</td><td>${plan?esc(`${fy}年度`):'—'}</td><td>${plan?esc(plan.itemCount??Object.keys(plan.rows||{}).length):'—'}</td><td><div class="dim-history">${plan?`${esc(plan.sourceMeta?.source_type||'LEGACY_PASTE')} · ${esc(plan.importedAt||'')}`:'—'}</div></td></tr>`;
