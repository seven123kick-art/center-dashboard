...
(function(){
  ...
  function buildData(ym){if(!window.CANONICAL_BUILDER||!window.DATA_VERIFICATION_SUMMARY)return null;const routeData=(STATE.routeData||[]).filter(x=>x?.ym===ym),snapshot=CANONICAL_BUILDER.buildSnapshot({routeData});const legacyMaster={workers:[...(STATE.workerMaster||[])],companies:[...(STATE.companyMaster||[])],center:window.CENTER||null};const resolutionContext=window.RESOLUTION_PREVIEW?.buildContext?.(Object.assign({},legacyMaster,{resolutionDecisions:persistedResolutionDecisions}))||null;return {snapshot,summary:DATA_VERIFICATION_SUMMARY.build({center_id:window.CENTER?.id||null,year_month:ym,canonical_snapshot:snapshot,source_status:sourceStatus(ym),accounting_facts:[],accounting_reconciliations:[]}),detail:window.DATA_VERIFICATION_DETAIL?.build({center_id:window.CENTER?.id||null,year_month:ym,canonical_snapshot:snapshot,worker_csv_data:STATE.workerCsvData||[],resolution_context:resolutionContext})||null};}
  ...
  async function saveResolution(){
    ...
    try{
      ...
      resolutionMessage={type:'success',text:'保存・Cloud再取得確認・Alias再構成・Resolver再評価が完了しました。'};
      render();
  ...
  window.DATA_VERIFICATION_UI=Object.freeze({render,buildSummary,setDetail,previewResolution,saveResolution,refreshResolution:()=>ensureResolutionLoaded(true)});
})();
