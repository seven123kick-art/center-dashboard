...
(function(){
  ...
  function categoryFor(label){const s=header(label);if((window.CONFIG?.INCOME_KEYS||[]).includes(s)||(window.CONFIG?.INCOME_SUB_KEYS||[]).includes(s))return'REVENUE';if((window.CONFIG?.LABOR_KEYS||[]).includes(s)||s==='運行旅費')return'LABOR';if((window.CONFIG?.YOSHA_KEYS||[]).includes(s))return'YOSHA';if((window.CONFIG?.EXPENSE_KEYS||[]).includes(s))return'OTHER_EXPENSE';return null;}
