...
(function(){
  ...
  const SALES_DETAIL = {
    ...
    fields: {
      ...
      center_id: field('string', false),
      attribution_subject_type: field('string', false, 'PERSON/ORGANIZATION/OPERATION_UNIT/PROCESS/UNKNOWN。SOURCEの「作業者」欄は人物専用ではなく売上・支払の帰属主体を表す'),
      attribution_subject_id: field('string', false, '解決済み帰属主体ID。主体種別に応じてWORKER/COMPANY/CENTER等へ接続。未解決ならnull'),
      worker_id: field('string', false, 'attribution_subject_type=PERSONの場合のみ。ROUTE_WORKER（実運行参加者）とは別概念'),
      worker_company_id: field('string', false, 'PERSONの配送/売上当時所属会社、またはORGANIZATION帰属先会社。SOURCE/MASTERで確定できる場合のみ'),
      attribution_center_id: field('string', false, 'OPERATION_UNITがセンターへ帰属する場合のCENTER_MASTER参照。推測で設定しない'),
      process_code: field('string', false, 'PROCESS主体の処理コード。D2-6ではSOURCE表記からの自動分類ルールを固定しない'),
      year_month: field('string', false),
      ...
      source_subject_label: field('string', false, 'WORKER_SALESの「作業者」原値。人物・会社・拠点・処理のいずれでもあり得るため原値を保持'),
      source_subject_company_name: field('string', false, 'WORKER_SALESの「協力会社名」原値'),
      subject_match_method: field('string', false, 'SUBJECT_RESOLVERが使用した照合方法。resolutionContext未指定時はnull'),
      subject_match_confidence: field('number', false, 'SUBJECT_RESOLVERの照合信頼度。未照合/未実行時はnull'),
      subject_resolution_status: field('string', false, 'OK/PROVISIONAL/UNMATCHED/CONFLICT等。DATA_QUALITYとは別の照合状態'),
